...
#     $SearchResults = @($SearchResults | ? {$.Operation -notlike 'FolderBind'})
#     write-host -fore green Filtered to $SearchREsults.Count Entries
#     }
#     $SearchResults = @($SearchResults | select ($LogParameters + @{Name='Subject';e={if (($.SourceItems.Count -eq 0) -or ($.SourceItems.Count -eq $null)){$.ItemSubject} else {($.SourceItems[0].SourceItemSubject).TrimStart(' ')}}},
#     @{Name='CrossMailboxOp';e={if (@('SendAs','Create','Update') -contains $.Operation) {'N/A'} else {$.CrossMailboxOperation}}}))
...
#     $SearchResults = @($SearchResults | ? {$.Subject -match $Subject -or $_.Subject -eq $Subject})
#     write-host -fore green 'Filtered to $($SearchREsults.Count) Entries'
#     }
